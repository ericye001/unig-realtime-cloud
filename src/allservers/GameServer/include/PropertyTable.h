...
public:
    ...
    char* GetDirtyData(uint16& nSize, bool bCleanDirty = true)       // 返回值：Dirty属性项数目
    {
        ...
        char* pData = pBuf;
        PtrMemCpy(pData, &nDirty, sizeof(nDirty));
        pData += sizeof(nDirty);
        ...
        for( int i = 0; i < GetPropertyCount(); ++i )
        {
            ...
            if( m_Property[pInfo->nOffset - 1] == 1 )      // dirty
            {
                PtrMemCpy(pData, &(pInfo->nInd), sizeof(pInfo->nInd));
                pData += sizeof(pInfo->nInd);
                PtrMemCpy(pData, &(m_Property[pInfo->nOffset]), pInfo->nByte);
                pData += pInfo->nByte;
    ...
    void SetDirtyData(char* dd)
    {
        ...
        for( int i = 0; i < nDirty; ++i )
        {
            ...
            PtrMemCpy(m_Property + pInfo->nOffset, dd, pInfo->nByte);
            dd += pInfo->nByte;
        }
    }
public:
	...
	{					
		...
		if( pInfo == 0 )								
			return;									
		PtrMemCpy(m_Property + pInfo->nOffset, v, pInfo->nByte);	
        SetDirtyBitValue(pInfo, 1);
	}															
...
{																							\
    m_Property = (char*)m_allocator(__FILE__,0,__FUNCTION__,m_alloc_malloc,Class::m_BufLen);\
...
{																							\
	if( m_Property )																		\
    {                                                                                       \
        m_deallocator(__FILE__,0,__FUNCTION__,m_alloc_free,m_Property);                     \
		m_Property = 0;																		\
    }                                                                                       \
}																							\
