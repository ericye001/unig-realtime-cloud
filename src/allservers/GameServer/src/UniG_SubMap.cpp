...
void UniG_SubMap::Area_For_Each( const ObjTypeKey& type, const Vector3& vCenter, float fRadius, UniG_GameObject* pSrc, void* pParam, int nFlag, void (*pfnCallback)(UniG_GameObject*, UniG_GameObject*, void* pParam, int nFlag))
{
	int MinColNum = (int)((vCenter.x - fRadius) / m_fXRegionUnit);
	int MaxColNum = (int)((vCenter.x + fRadius) / m_fXRegionUnit);
	int MinRowNum = (int)((vCenter.z + fRadius) / m_fZRegionUnit);
	...
	for(i=MinRowNum; i<=MaxRowNum; i++)
	{
		for(j=MinColNum; j<=MaxColNum; j++)
		{
			...
			pR->For_Each(type, pSrc, pParam, nFlag, pfnCallback);
		}
	}
}

void UniG_SubMap::Sync_For_Each(const ObjTypeKey& type, UniG_GameObject* pSrc, void* pParam, int nFlag, void (*pfnCallback)(UniG_GameObject*, UniG_GameObject*, void* pParam, int nFlag))
{
	for(int i=0; i < m_nColRegionNum*m_nRowRegionNum; i++)
	{
		m_pRegionArrPtr[i].For_Each(type, pSrc, pParam, nFlag, pfnCallback);
	}
}

void UniG_SubMap::Sync_For_Each( const ObjTypeKey& type, const Vector3& vPos, UniG_GameObject* pSrc, void* pParam, int nFlag, void (*pfnCallback)(UniG_GameObject*, UniG_GameObject*, void* pParam, int nFlag) )
{
	...
	for( int i = -MAX_SYNC_REGION_LEVEL_NUM; i <= MAX_SYNC_REGION_LEVEL_NUM; ++i )
		for( int j = -MAX_SYNC_REGION_LEVEL_NUM; j <= MAX_SYNC_REGION_LEVEL_NUM; ++j )
		{
			...
			pField->For_Each( type, pSrc, pParam, nFlag, pfnCallback );
		}
}

void UniG_SubMap::BroadcastMsg( const ObjTypeKey& type, UniG_GameObject* pSrc, ObjectMessage* pMsg, bool bToSelf )
{
	for(int i=0; i < m_nColRegionNum*m_nRowRegionNum; i++)
	{
        m_pRegionArrPtr[i].For_Each(type, pSrc, pMsg, bToSelf, UniG_Player::ObjectTellPlayer);
	}
}

void UniG_SubMap::BroadcastMsg( const ObjTypeKey& type, const Vector3& vPos, UniG_GameObject* pSrc, ObjectMessage* pMsg, bool bToSelf )
{
	MapRegion* pRegion = GetRegion(vPos);
	if( !pRegion )
		return;
	...
	for( int i = -MAX_SYNC_REGION_LEVEL_NUM; i <= MAX_SYNC_REGION_LEVEL_NUM; ++i )
		for( int j = -MAX_SYNC_REGION_LEVEL_NUM; j <= MAX_SYNC_REGION_LEVEL_NUM; ++j )
		{
			pField = GetRegion(pRegion->m_nRow + i, pRegion->m_nCol + j );
            if( pField )
			    pField->For_Each(type, pSrc, pMsg, bToSelf, UniG_Player::ObjectTellPlayer);
		}
}
