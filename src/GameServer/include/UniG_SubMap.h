...
public:
	...
	// broadcast
	void Area_For_Each( const ObjTypeKey& type, const Vector3& vCenter, float fRadius, void* pParam, void (*pfnCallback)(UniG_GameObject*, void* pParam));
	void Sync_For_Each( const ObjTypeKey& type, void* pParam, void (*pfnCallback)(UniG_GameObject*, void* pParam) );
	void Sync_For_Each( const ObjTypeKey& type, const Vector3& vPos, void* pParam, void (*pfnCallback)(UniG_GameObject*, void* pParam) );

    void BroadcastMsg( const ObjTypeKey& type, ObjectMessage* pMsg );
	void BroadcastMsg( const ObjTypeKey& type, const Vector3& vPos, ObjectMessage* pMsg );

    void BroadcastMsg( const ObjTypeKey& type, const Vector3& vPos, ObjectMessage* pMsg, UniG_GameObject* pExclude);
    ...
    void SetRegionInfo(uint32* pRegionInfo, uint32 nRegionNum)
    {
        if(nRegionNum >0 && nRegionNum == m_nRowRegionNum * m_nColRegionNum)
        {
            ...
            memcpy(m_pRegionInfo, pRegionInfo, nRegionNum * sizeof(uint32));
        }
