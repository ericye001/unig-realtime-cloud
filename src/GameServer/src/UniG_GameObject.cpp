...
int UniG_GameObject::GetType() 
{
    return m_nType;
}

void UniG_GameObject::SetType(int nType)
{
    m_nType = nType;
}
...
bool UniG_GameObject::IsValid(void) const
{
	return (m_nGuid != 0xffffffff);
}
...
void UniG_GameObject::Serialize(AgentCreateMessage* pMessage)
{
    ...
    memcpy(pMessage->m_pData, m_Property, pMessage->m_nLength);   
}
...
void UniG_GameObject::DeSerialize(AgentCreateMessage* pMessage)
{
    ...
    memcpy(m_Property, pMessage->m_pData, pMessage->m_nLength);
}
...
void UniG_GameObject::SendMsgToNeighbour(ObjectMessage* pMessage, bool bIncludeSelf)
{
    ...
    if(bIncludeSelf)
    {
        m_pSubMap->BroadcastMsg(m_nType, GetPosition(), pMessage);
    }
    else
    {
        m_pSubMap->BroadcastMsg(m_nType, GetPosition(), pMessage, this);
    }
...
void UniG_GameObject::SendMsgToNeighbour(ObjectMessage* pMessage, Vector3& vCenter, float fRadius)
{
    Assitant(pMessage);
    // construct a result message to send to agent    
    m_pSubMap->BroadcastMsg(m_nType, pMessage);
}
...
void UniG_GameObject::Assitant(ObjectMessage* pMessage)
{
	...
	memcpy(synMsg.m_pMsgData, pMessage->m_pMsgData, synMsg.m_nMsgLength);
    ...
    for(iter = srvList.begin(); iter != srvList.end(); iter++)
    {
        ...
        printf("send syn out messag to agent");
    }
}
...
void UniG_GameObject::SyncProperty()
{
    ...
    for(iter = srvList.begin(); iter != srvList.end(); iter++)
    {
        ServerContextMgr::Instance()->SendBlock(*iter, pkt.Data(), pkt.Length());
    }
}
